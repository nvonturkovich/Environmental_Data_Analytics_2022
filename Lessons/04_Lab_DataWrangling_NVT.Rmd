...
NTL.phys.data.tuesday_1990_99 <- filter(NTL.phys.data, lakename == "Tuesday Lake", year4 %in% c(1990:1999)) 
NTL.phys.data.tuesday_1990_99 
...
NTL.phys.data.Tues_July_90_99_surface<-NTL.phys.data.Tues_July_90_99 %>% 
  filter(depth == 0) %>% 
  filter(!is.na(temperature_C))%>% #alternatively dropna(temperatureC)
  summarise(meantemp= mean(temperature_C))

summary(NTL.phys.data.Tues_July_90_99_surface$temperature_C)
