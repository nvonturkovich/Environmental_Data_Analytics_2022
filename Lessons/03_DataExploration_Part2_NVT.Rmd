...
```{r, fig.height = 3, fig.width = 4}
# you can put the aes in the geom() or oustide, same same
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = discharge.mean))

# 30 bins is default, use between 5-20
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = discharge.mean), binwidth = 10)

# bins = same as setting the bin width
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = discharge.mean), bins = 20)

# scale, zooms in on data, omits some values
ggplot(USGS.flow.data, aes(x = discharge.mean)) +
  geom_histogram(binwidth = 10) + 
  scale_x_continuous(limits = c(0, 500))
  
# showing a different value (gage height mean)
ggplot(USGS.flow.data) +
...
# like a histogram but with box plots. thats what the cut width is
ggplot(USGS.flow.data) +
  geom_boxplot(aes(x = gage.height.mean, y = discharge.mean, group = cut_width(gage.height.mean, 1)))

# violin plot, range and distribution within hte range - much more descriptive
ggplot(USGS.flow.data) +
...
> Answer: 
breaks in data, how the data is distributed, outliers


How can multiple options for data exploration inform our understanding of our data?

> Answer: 
different plots and graphs and reveal different aspects of the data you couldnt see before.
