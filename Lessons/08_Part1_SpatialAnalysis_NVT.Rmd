...
GeoJSON - data sets with geo points
sf object = spatial analysis object
...
#View using ggplot
ggplot(data=GasPlants) + geom_sf() #map, points in geographic space
...
EPAair <- read.csv("./Data/Processed/EPAair_O3_PM25_NC2021_Processed_NVT.csv", stringsAsFactors = TRUE) %>% 
...
sfc_GraingerHall <- st_sfc(sfg_GraingerHall,crs=4326) #crs links it with a cord system, cord ref systems. epsg.io type in code to look up the cord system
...
lat long to cartrsian =projection
...
the_buffer <- sf_GraingerHall %>% st_buffer(1)#1 degree, in coords but looks oval because this is a plane not a sphere
...
EPSG are for defined cord systems
...
EPAair.sf.utm <- st_transform(EPAair.sf,32617) #sftransofrm - transforms cord system
