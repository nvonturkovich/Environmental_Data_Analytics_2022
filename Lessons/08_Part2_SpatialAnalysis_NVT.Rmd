...
epa_pm25_sites_sf <- read_csv('./Data/Raw/EPAair_PM25_NC2018_raw.csv') %>% 
  ...
  st_as_sf(coords = c('SITE_LONGITUDE','SITE_LATITUDE'), crs=4269)#spatial data frame
...
st_bbox(counties_sf)#cordinates of spatial bounding box
...
huc8._sf.subset<- st_read('./Data/Spatial/NCHUC8.shp')%>% 
  filter(SUBBASIN == 'Upper Neuse') #Filter for just NC Counties
head(huc8.2_sf)
...
ggplot()+
  ...
  geom_sf(data=huc8._sf.subset, fill = 'purple')
...
wind_energy_huc8<-read.csv('https://raw.githubusercontent.com/ENV859/EnviroAtlasData/main/Wind_Energy.csv', 
                           colClasses = c('HUC_12' = 'factor')) %>% 
  mutate(HUC_8= substr(HUC_12,1,8)) %>% #speficy a string, pulls first 8 characters out
  group_by(HUC_8) %>% 
  summarize(kWhkm2day = sum(kWhkm2day))

head(wind_energy_huc8)

#Join to HUC_8 features
#huc_sf_wind.2 <-huc8_sf %>% 
  #left_join(huc8_sf, wind_energy_huc8, by = c('HUC_8'= 'HUC_8'))
  
huc_sf_wind <- merge(x=huc8_sf, 
                     y=wind_energy_huc8,
                     by.w="HUC_8",
                     by.y="HUC_8")
head(huc_sf_wind)
...
#OR
ggplot() + 
  geom_sf(data=huc_sf_wind,aes(fill = kWhkm2day),alpha=0.3) + 
  scale_fill_gradient2(low="red",high="blue",midpoint = 60000) + 
  geom_sf(data=huc_sf_wind)
...
its better to convert to planar systems!
...
st_crs(epa_sf_utm)
#ANGLEUNIT = degrees=geographic
#  or ID["EPSG",26917] - search this EPSG number and it will tell you
```
...
triangle_HUCs.clipped<- triangle_co %>% 
  st_intersects(huc8_sf_utm) #huc 8s=cookie dugh, cookie cutter = triangle_co
