...
matrix1
matrix2 <- matrix(1:20, nrow = 5, ncol = 4, byrow = TRUE) #organized by row, FALSE would be organized by column
matrix2
...
matrix1[4, ] # fourth row, all number 
matrix1[ , 3] # 3rd column
matrix1[c(12, 14)] # specific coord
matrix1[c(12:14)] # slice of the points from 12 to 14
matrix1[2:4, 1:3] # 2 slices
...
list1[[2]]
list$name #pull up items in the collection list by key ($)


# Data Frames ---- creating three vectors, d,e,f
d <- c(1, 2, 3, 4) # What type of vector?
e <- c("red", "white", "red", NA) # What type of vector?
f <- c(TRUE, TRUE, TRUE, FALSE) # What type of vector?
dataframe1 <- data.frame(d,e,f) # can also use c_bind to bind vectors into a data frame
names(dataframe1) <- c("ID","Color","Passed"); View(dataframe1) # rename column names
#rows are observations, columns are properties of the observation

dataframe1[1:2,] # 
dataframe1[c("ID","Passed")] # show only columns ID and Passed
dataframe1$ID # just pulling ID column
...
```{r}
lowtemps_F<-c(20, 25, 28, 19, 23, 25, 29, 43, 49, 36)
hightemps_F<-c(41, 43, 46, 36, 42, 49, 52, 56, 67, 60)
```

Now, create two additional vectors that include the ten-day forecast for the high and low temperatures in Celsius. Use a function to create the two new vectors from your existing ones in Fahrenheit. (F-32)*(5/9)=C

```{r}
F_C<-function(x){((x-32)*(5/9))}
lowtemps_C<-F_C(lowtemps_F)
hightemps_C<-F_C(hightemps_F)


```

Combine your four vectors into a data frame and add informative column names. 10 rows, 4 columns

```{r}
temps.df<-data.frame("Lows F"=lowtemps_F, "Highs F"=hightemps_F, "Lows C"=lowtemps_C, "Highs C"=hightemps_C) 
temps.df<-round(temps.df, 2)
temps.df
```
