...
BRA_Temperature_C_ts <- ts(BRA_complete$Temperature_C, start = c(fyear,fmonth), frequency = 12)
...
Help here - not periodic??
```{r}

# Exercise 8: Use the stl() function to decompose both series. # Generate the decomposition
# Visualize the decomposed series using plot(). What components can you identify?

BRA_Rainfall_Decomposed <- stl(BRA_Rainfall_ts, s.window = "periodic") #pulls apart data
plot(BRA_Rainfall_Decomposed)

BRA_Temp_Decomposed <- stl(BRA_Temperature_C_ts, s.window = "periodic") #pulls apart data
plot(BRA_Temp_Decomposed)
#trend is the random part of the series
...
BR_Rain_Components <-
  as.data.frame(BRA_Rainfall_Decomposed$time.series[,1:3])
BR_Temp_Components <-
  as.data.frame(BRA_Temp_Decomposed$time.series[,1:3])
#need to convert timeseries to df

#adds time series back to orginal df
...
Rain_trend <- trend::smk.test(BRA_Rainfall_ts)
#inspect results
Rain_trend
summary(Rain_trend)
