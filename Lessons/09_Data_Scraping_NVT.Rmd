...
1. look to see if the site allows direct download
2. Look for an API- programaticallly access the data - an r plug in to access data. example census data
3. look to see if they allow scraping - look at terms of services or robots.rxt to see if they allow it

Robots:
you can find out if a site allows scraping or not by using robots.txt
go to the base url and add robots.txt
www.nytimes.com/robots.txt if it says "disallow" dont scrape it
...
df_withdrawals <- data.frame("Month" = rep(1:12), #  creates a sequence of numbers
                             "Year" = rep(2000,12), # 12 repeaes of the numner 2000
                             "Avg_Withdrawals_mgd" = as.numeric(avg_withdrawals), #changes scraped data into numeric
...
scrape.it <- function(the_year, the_facility){
  ...
  Sys.sleep(5) #uncomment this if you are doing bulk scraping! good eticate to wait 5 seconds, lets website know they are not under attack
...
the_dfs <- lapply(X = the_years, #base R. run the function FUN for each year in "the years, while also looking for "my facility", my facility is static
                  FUN = scrape.it,
                  the_facility=my_facility)

#OR, use purrr's map function
the_dfs <- map(the_years,scrape.it,the_facility=my_facility) #does same thing at lapply, but uses tidy map function

#Conflate the returned dataframes into a single dataframe
the_df <- bind_rows(the_dfs) #bind row is tidy version of r bind
...
the_years <- rep.int(2020,length(the_facility_ids_subset)) #need to have same size list

#"Map" the "scrape.it" function to retrieve data for all these
dfs_2020 <- map2(the_years, the_facility_ids_subset, scrape.it) #iterate through both variables in parallel, year and site in pairs
...
#Plot
ggplot(df_2020,aes(y = Avg_Withdrawals_mgd, x=Facility_type)) + # x=as.factor(month), 
  geom_boxplot()
...
the_years = c(2018,2019,2020) #this would pull 2018 for site 1, 2019 for site 2 and 2020 for site 3, we dont want that
...
the_df <- cross2(the_years,the_facilities) %>% #generates list of lists
  map(lift(scrape.it)) %>% #lists unique combinations
  bind_rows()
