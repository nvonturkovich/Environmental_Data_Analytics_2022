...
#Read Eno river data
EnoDischarge <- read.csv("./Data/Processed/USGS_Site02085000_Flow_Processed_NVT.csv",
...
short, medium, long
trend- grow or decline over time
cycle-business phase, ENSO
seasonal variation-occures more periodically than a cycle - climate - weather
random variations-cant be predicted-strike, war, fire

Terms:
stationary data-no upward or downward trend over tinme
nonstationary-a time series variable exhibiting a sig upward or downward trend over time
seasonal data - repeating patterns at regular internvals over time

correlation-joint variablity or measure of linear dependence between two variables
autocorrelation(TSA)-measure of dependence between two adjacent values of the same variables. auto = self correlation

auto-covariance - ACF-can identify seasonality on a data set
Yt=original series
Ys=lagged verison, comparing Y2 with Y1
can lagged by more than one variables Y3 with Y1
stationary Process-probabilty law govern the behvaior of hte process(mean, SD etc) do not change over time
white noise series-sequence of independent identically distributed random variablse
partial autocorrelation PACF-remove influcence of other intermediaute observations

always build ACF and PACF when you try fit a model
autoregressive component-TSA based on previous data

linear trend component-increase(upward), decreasing (downward)
detrend-for each observatoin t remove trend (B0+B1t)
non-linear trend-ex. quandratic
exponential trend - log of series y
*most of the time we assume a linear trend

Seasonal Component:
observed within 1 year
u(mu)t-can take 12 different values or t's depending on the t
subtract seasonal component

Determinastic = linear trend =polynomical trend. linear regression. residuals will give you the detrended series = trend sationarity
stochastic trend - difference stationarity - distant...

stationarity test -
Mann-Kendall - detect deterministic trends (not seasonal)
spearmans correlation coefficient-strength of a monotonic relationship - can measure not linear relationships. cor.test() - need to change setting
dick-fuller test- white noise-verifys  if they series has a unit root. ADF test
...
important:
x - univartiate
s.window - periodicity

```{r}
?stl
# Generate the decomposition
EnoDischarge.late_Decomposed <- stl(EnoDischarge.late_ts, s.window = "periodic") #pulls apart data
...
EnoDischarge.late_Components <- mutate(EnoDischarge.late_Components,
        Observed = EnoDischarge.late$discharge.mean,  #add column   
        Date = EnoDischarge.late$datetime) #add column
...
#cant just remove NA's, you need to replace the NA with na.approx
```
...
#p value <.05 then we do have a trend
...
> Answer:there is a decrease trend of overage wind speed over time
...
You can have more than one seasonal components _ nightly, monthly
