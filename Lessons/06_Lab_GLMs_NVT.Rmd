...
EPAair <- read.csv("./Data/Processed/EPAair_O3_PM25_NC1819_Processed_NVT.csv", stringsAsFactors = TRUE)
# Set date to date format
...
EPAair.subsample <- sample_n(EPAair, 5000) # sample n takes random subset of data


# Evaluate assumption of normal distribution
shapiro.test((EPAair.subsample$Ozone)) #p value is low - that means data is not normal
ggplot(EPAair, aes(x = Ozone)) + #okay so take a look at the plot, 
  geom_histogram() 
...
null: difference between the sample mean and the value is 0
alt: the difference is NOT zero (two sided)
