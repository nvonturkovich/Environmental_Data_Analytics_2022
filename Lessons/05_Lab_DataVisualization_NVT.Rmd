...
ggplot(data=data.frame(x=0,y=0))+
  ...
  geom_text(x=0,y=0.025,label=expression(paste("Mean is ", mu,",", R^{2},'=0.6'))) #how to represent labels, ex. pi, mu, degree
print(Plot_text)
...
PeterPaul_Plot<-ggplot(filter(PeterPaul.chem.nutrients, depth==0)) + #filter for just surface temps
  geom_point(aes(x = daynum, y = temperature_C, color = year4))+ # better to put the aes in the geom
  facet_wrap('lakename')+ #facet allows makes different pains, alternate option facet_wrap(vars(lakename), ncol =2) #facet_warp or facet_grid
  ylab('Temperature (C)')+
  xlab('Day Number of Year')
